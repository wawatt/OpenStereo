...
def context_upsample(disp_low, up_weights, scale_factor=4):
    ...
    b, _, h, w = map(int, disp_low.size())
    disp_unfold = F.unfold(disp_low, kernel_size=3, dilation=1, padding=1)  # [bz, 3x3, hxw]
    disp_unfold = disp_unfold.reshape(b, 9, h, w)  # [bz, 3x3, h, w]
    disp_unfold = F.interpolate(disp_unfold, scale_factor=scale_factor, mode='nearest')  # [bz, 3x3, 4h, 4w]
